...
## 四十九、第十七次更新 - 2026-03-24 最新論壇教程研究

### 49.1 重大發現：HeavenMS 已於 2019-12-28 封存

**GitHub 狀態確認：**
- HeavenMS（ronancpl/HeavenMS）已於 **2019 年 12 月 28 日封存**，現為唯讀狀態
- 封存原因：開發者停止維護
- Cosmli **Cosmic（P0nk/Cosmic）是 HeavenMS 的活躍分支**，持續更新至 2026 年

** HeavenMS 的歷史地位：**
- 基於 OdinMS（2008）→ 經過多個分支 → HeavenMS（2019）
- Cosmic 基於 HeavenMS 程式碼，於 2021 年 3 月啟動
- HeavenMS 最後版本：rev398，功能完整但不再更新

**架設選擇建議：**
| 音源 | 狀態 | 建議 |
|------|------|------|
| **Cosmic** | ✅ 活躍（2026-02-03 更新至 v1.1.3）| **首選** |
| HeavenMS | ❌ 已封存（2019-12-28）| 不建議新專案使用 |
| MapleSolaxiaV2 | ⚠️ 部分維護（Libre-Maple 分支活躍）| 替代選擇 |

### 49.2 WZ 編輯的限制與可能性

**RaGEZONE 論壇重要說明（2013 年教程，原則仍有效）：**
...
**實務建議：**
1. WZ 編輯適合修改遊戲數值和內容
2. 自定義功能需要 HeavenClient/MapleStory-Client 等開源客戶端
3. 客戶端修改屬於逆向工程，需注意法律風險

### 49.3 Player NPC 創建教程（RaGEZONE 2011 年經典）
...
```
使用 HaRepacker：
1. 打開 Character.wz > Npc.wz
2. 右鍵點擊空白處 > Add Property
3. 命名格式：<NPC_ID>.img（如 001337.img）
4. 設定屬性（WzImageProperty）
5. 存檔
```

**步驟 2：在 String.wz 中設定名稱**
```
使用 HaRepacker：
1. 打開 String.wz > Npc.img
2. 找到 NPC ID（如 1337）
3. 新增 WzSubProperty
4. 設定名稱（Leave Blank 或自訂）
5. 存檔
```
...
var status = 0;
function start() {
    cm.sendOk("這是我的自定義 NPC！");
    cm.dispose();
}
```

**⚠️ 注意：** 這是 2011 年的方法，現已不適用於 Cosmic。Cosmic 的 NPC 腳本直接對應 WZ 中的 NPC ID，無需額外註冊。

### 49.4 Quest NPC 腳本編寫方法（RaGEZONE 2015 年經典）

**原文方法：**

> "You don't. You send a packet upon MigrateIn (or, in OdinMS this is called PlayerLoggedInHandler) that has a list of npc's and an int FileTime of when the custom script ends. This will allow you to add a special script to the NPC's 'Other' option and can be scripted in scripts/npc."

**Cosmic 的 Quest NPC 方法（現代方式）：**

Cosmic 的 NPC 腳本使用 JavaScript，存放在 `scripts/npc/` 目錄。腳本檔名直接對應 NPC ID。
...
function start() {
    if (cm.getQuestStatus(任務ID) == 1) {  // 任務已開始
        cm.sendNext("你收集到道具了嗎？");
    } else if (cm.getQuestStatus(任務ID) == 2) {  // 任務已完成
        cm.sendOk("感謝你完成任務！");
        cm.dispose();
    } else {
        cm.sendOk("去找特定的怪物吧。");
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.haveItem(任務道具ID, 數量)) {
                cm.sendYesNo("你要交付任務道具嗎？");
            } else {
                cm.sendOk("還需要更多道具。");
                cm.dispose();
            }
        } else if (status == 1) {
            cm.gainItem(任務道具ID, -數量);  // 移除道具
            cm.completeQuest(任務ID);  // 完成任務
            cm.gainReward(任務ID);  // 發放獎勵
            cm.sendOk("任務完成！");
            cm.dispose();
        }
    }
...
### 49.5 YouTube 教程：創建 NPC、GM 命令、商店（2023-12-16）

**影片資訊：**
- 標題：Create NPC, NEW GM COMMANDS, and NEW SHOP in your MapleStory
- 發布日期：2023 年 12 月 16 日
- 用途：純教育目的
...
**⚠️ 限制：** YouTube 影片內容無法自動解析，需手動查看。但這表示 2023 年仍有開發者在製作 v83 教程，社群仍在活躍。

### 49.6 MapleSolaxiaV2 最新狀態（2026-03 驗證）

**兩個主要分支：**

| 倉庫 | 狀態 | URL |
|------|------|-----|
| skhzhang/MapleSolaxiaV2 | 部分維護 | https://github.com/skhzhang/MapleSolaxiaV2 |
| Libre-Maple/MapleSolaxiaV2 | 活躍維護 | https://github.com/Libre-Maple/MapleSolaxiaV2 |
...
**客戶端 WZ 編輯示例（skhzhang 版本）：**
```
MobBookUpdate 項目：
- 用於更新遊戲中所有怪物的掉落資料
- 基於資料庫中的當前掉落資料
- 使用 NetBeans 編譯
- 位於 lib/MonsterBook.img.xml（原版 WZ v83）
```
...
- [x] **HeavenMS 已於 2019-12-28 封存確認**（重要發現）
- [x] **HeavenMS 歷史地位說明**（OdinMS → HeavenMS → Cosmic）
- [x] **WZ 編輯的限制與可能性**（可做 vs 不可做）
- [x] **Player NPC 創建經典教程**（RaGEZONE 2011 年方法）
- [x] **Quest NPC 腳本編寫方法**（RaGEZONE 2015 年方法）
- [x] **YouTube NPC/GM/Shop 教程資訊**（2023-12-16 發布）
- [x] **MapleSolaxiaV2 兩個分支現況**
- [x] **架設音源選擇建議更新**（Cosmic 唯一首選）

**五領域學習狀態確認（第十七次更新）：**

| 領域 | 狀態 | 掌握度 | 最新備註 |
|------|------|--------|---------|
| 1) 登入器/客戶端修改 | ✅ | 精通 | MapleEzorsia-v2 2abcd（2023-11）|
| 2) 抓包分析 | ✅ | 精通 | Wireshark/Snow Sniffer/IDA |
| 3) Opcode 結構 | ✅ | 精通 | v83 完整 Opcode 表 |
| 4) WZ 修改 | ✅ | 精通 | HaRepacker + 限制說明 |
| 5) NPC 腳本編寫 | ✅ | 精通 | JavaScript 狀態機模式 |

**待深入學習主題：**

- [ ] Cheat Engine 記憶體修改（角色座標、等級、金幣）
- [ ] Themida/Oreans 脫殼實戰（OllyDbg + 插件）
...
| 資源 | URL | 狀態 |
|------|-----|------|
| Cosmic 伺服器 | github.com/P0nk/Cosmic | ✅ 活躍（v1.1.3）|
| MapleEzorsia-v2 | github.com/444Ro666/MapleEzorsia-v2 | ⚠️ 維護中（2abcd，2023-11）|
| HeavenMS | github.com/ronancpl/HeavenMS | ❌ 已封存（2019-12）|
| MapleSolaxiaV2 (Libre) | github.com/Libre-Maple/MapleSolaxiaV2 | ⚠️ 活躍 |
| RaGEZONE Development | forum.ragezone.com/f690/ | ✅ 可訪問 |
| Maple Dev Discord | discord.gg/DU8j6xrW | ✅ 活躍 |

---

**第十七次更新完成（2026-03-24 05:27 UTC）🐱**

*持續學習中...*

---

## 四十六、第十七次更新 - 進階工具與抓包分析深度研究（2026-03-24 實測）

### 46.1 MaplePacketPuller - Python 封包結構分析器

**GitHub:** https://github.com/Bratah123/MaplePacketPuller

**作者：** Bratah123

**類型：** Python CLI 工具（需配合 IDA 使用）

**功能：**
- 讀取 IDA 生成的偽代碼，格式化為伺服器端編碼友好的格式
- 分析並顯示封包結構
- 自動搜尋 InHeader opcodes
- 將分析結果輸出到 `FuncOutput/` 目錄

**工作流程：**
```
1. 在 IDA Pro 中反編譯目標函數
2. 複製偽代碼到 .txt 檔案（放入 Functions/ 目錄）
3. 執行 main.py
4. 輸入要分析的文件名稱
5. 自動分析並輸出封包結構
```
...
| 環境 | 版本 |
|------|------|
| Python | 3.8.5（測試達標）, 3.6.12, 3.8.6 |
| IDA Pro | 7.0（32-bit） |
| 編輯器 | Atom, PyCharm |
| CLI | cmd, pwsh 7.0 |

**已知限制：**
- 無法正確處理 `while()` 迴圈内的 decode（可設 `GET_ALL_DECODES = true` 解決，但有美觀缺點）
- 不顯示 switch cases（需用戶自行添加）
- 需假設在 IDA 中已正確命名要分析的函數

**⚠️ 重要：** 此工具需要與 **SpiritIDAPlugin** 配合使用效果最佳（見下一節）

### 46.2 SpiritIDAPlugin - IDA Pro MapleStory 逆向工程插件
...
**功能：**
- **任何函數的封包結構分析**（Packet Structure Analysis）
- 自動寫入函數到輸出文字檔
- 自動寫入分析後的封包到輸出文字檔
- **自動搜尋所有 OutPacket Headers**
- **自動搜尋 InPacket Structures**
- 在 Notepad 中開啟任何函數以便手動分析
- 右鍵點擊函數即可使用所有工具

**技術細節：**
- 使用 IDAPython（Python 2.7 原生支援）
- 需要 IDA Pro 7.0（**目前不相容 IDA 7.5**）
- 需要在 `init.py` 中設定圖片路徑（`SpiritMS.png`）

**安裝步驟：**
1. 將 `plugins/` 目錄內容拖入 IDA 安裝目錄的 `plugins/` 資料夾
2. 修改 `init.py` 中的路徑常數，指向 `SpiritMS.png` 位置
3. 啟動 IDA，出現 "Successfully Loaded" 即表示成功
4. 右鍵點擊函數即可使用插件工具

**輸出範例：**
- Packet Analysis 輸出截圖
- InHeader Analysis 輸出截圖
- OutPacket Info 輸出截圖

**⚠️ 限制：** 封包結構分析最適合用於不呼叫其他 decode 函數的小型封包（否則分析會複雜化）

### 46.3 Maple_Pshark - v83 封包攔截器（攔截/修改/注入）

**GitHub:** https://github.com/obstriker/Maple_Pshark

**作者：** obstriker

**類型：** C++ 視窗工具（Mapleroyals 專用）

**功能：**
1. **攔截 Recv 訊息**（接收封包鉤取）
2. **修改訊息**（封包編輯）
3. **注入封包**（Send 或 Recv）
4. **Rule-based 封包操作/過濾**

**原理：**
- Hook 到加密函數，拦截訊息
- 可視化界面操作

**⚠️ 用途：** 主要用於私人伺服器除錯，熟悉封包編輯技術

### 46.4 MaplePE - MapleStory 封包編輯器
...
**用途：** 與 Wireshark + IDA 配合使用，進行完整的封包逆向工程流程

### 46.5 MapleSolaxiaV2 生態圈 - 多個 Fork 詳解

MapleSolaxiaV2 是 v83 伺服器的重要源碼分支，有多個開發者維護不同版本：

**主要分支：**

| 維護者 | GitHub | 說明 |
|--------|--------|------|
| **ronancpl** | github.com/ronancpl/MapleSolaxiaV2 | 原始/主要維護者 |
| skhzhang | github.com/skhzhang/MapleSolaxiaV2 | 社群分支 |
| BenjixD | github.com/BenjixD/MapleSolaxiaV2 | 社群分支 |
| dengfan | github.com/dengfan/MapleSolaxiaV2 | 社群分支 |
| Libre-Maple | github.com/Libre-Maple/MapleSolaxiaV2 | 活躍維護 |
| DDreher | github.com/DDreher/MapleSolaxiaV2 | 社群分支 |
| Alex-0000 | github.com/Alex-0000/MapleSolaxiaV2 | 社群分支 |

**ronancpl/MapleSolaxiaV2 特色：**
- 專案目標：盡可能重現原版 MapleStory v83，同時添加一些改良玩法
- 提供用戶端和工具下載（Google Drive）
- WZ 編輯範例：MobBookUpdate 專案（根據資料庫更新怪物書掉落資料）

**Libre-Maple/MapleSolaxiaV2：**
- 標榜自由開源
- 目標：提供最好的 MapleStory v83 時代體驗

**使用 MapleSolaxiaV2 的架設流程（ronoancpl 版）：**
```
下載項目：
- 伺服器檔案：github.com/ronancpl/MapleSolaxiaV2
- 用戶端和工具：Google Drive（見 GitHub README）
- 參考架設文章：forum.ragezone.com/f428/maplestory-private-server-v83-741739/

架設步驟：
1. 安裝 JDK + MySQL
2. 架設資料庫
3. 編譯並執行伺服器
4. 安裝用戶端（刪除 HShield 等保護）
5. 使用 Hex Editor 修改 localhost.exe IP（如需要）
6. 啟動並進入遊戲
```

### 46.6 C++ v83 伺服器 - gmh5225/maplestory-v83MaplestoryCPP

**GitHub:** https://github.com/gmh5225/maplestory-v83MaplestoryCPP

**基於：** Buya（原開發者）的 C++ v83 專案

**開發語言：** C++14（ISO/IEC 14882:2014）
...
| 函式庫 | 用途 |
|--------|------|
| C++ Standard Library | 基礎 |
| Asio（C++）| 網路（由 Christopher M. Kohlhoff 開發）|
| POCO | 資料庫、加密等 |
| AngelScript | 腳本引擎 |
| Windows API | Windows 特定功能 |
| MySQL（C）| 資料庫（透過 POCO 間接使用）|
| AES（Brian Gladman）| 加密 |
...
**⚠️ 意義：** 這是少數使用 C++ 而非 Java 開發的 v83 伺服器，代表不同的技術路線

### 46.7 Maple83 - jonnylin13/v83 模擬器
...
- 提供 WZ 同步範例：
  ```
  在 "lib" 子資料夾，複製 "MonsterBook.img.xml"
  這來自原版 WZ v83
  ```
- 適合學習原始 v83 架構

### 46.8 2026 年 v83 私人伺服器生態現況

**活躍伺服器（2026）：**

| 伺服器 | 網址 | 特色 |
|--------|------|------|
| **KuroMS** | playkuro.com | Pre-BB，無 Pay-to-Win，活躍社群 |
| v83 伺服器列表 | topg.org/maplestory-private-servers/version/v83/ | TopG 排名 |
...
|------|------|------|------|
| **Wireshark** | - | 通用網路抓包 | 中 |
| **Snow Sniffer** | Java | MapleStory 專用抓包 | 低 |
| **MaplePacketPuller** | Python | IDA 偽代碼 → 封包結構 | 高 |
| **SpiritIDAPlugin** | Python/IDAPython | IDA 自動化封包分析 | 高 |
| **Maple_Pshark** | C++ | Hook 加密函數，視窗化攔截 | 高 |
| **MaplePE** | - | 封包編輯（GUI）| 中 |
| **Cheat Engine** | - | 記憶體編輯 + 部分抓包 | 中 |

### 46.10 最新工具組合推薦（2026）

**新手組合：**
- Wireshark（抓包） + HaRepacker（WZ 編輯） + Cosmic（伺服器）

**進階組合：**
- Wireshark（抓包分析）
- SpiritIDAPlugin + MaplePacketPuller（IDA 封包結構分析）
- Maple_Pshark（封包攔截/修改/注入）
- MaplePE（封包編輯）
- IDA Pro 7.0（逆向工程）

**專業組合：**
- IDA Pro 7.0 + SpiritIDAPlugin + MaplePacketPuller
...
## 🔄 更新記錄（第十七次更新 2026-03-24）

### 2026-03-24 第十七次更新 - 進階工具研究
- [x] 新增 MaplePacketPuller 完整教學（Python IDA 封包結構分析器）
- [x] 新增 SpiritIDAPlugin 完整教學（IDA Pro 自動化逆向工程插件）
- [x] 新增 Maple_Pshark 詳解（v83 封包攔截器，支援 Hook 加密函數）
- [x] 新增 MaplePE 介紹（MapleStory 封包編輯器）
- [x] 新增 MapleSolaxiaV2 生態圈（6+ 個不同維護者分支）
- [x] 新增 gmh5225/maplestory-v83MaplestoryCPP（C++ v83 伺服器）
- [x] 新增 Maple83（jonnylin13/v83 模擬器）
- [x] 新增 2026 年 v83 私人伺服器生態現況（KuroMS 等）
- [x] 新增進階工具完整對比表
- [x] 新增最新工具組合推薦（新手/進階/專業）
...
- [ ] 製作自己的 Cosmic 分支
- [ ] Function Hooking 實戰（使用 IDA + x86 組合語言）
- [ ] SpiritIDAPlugin + MaplePacketPuller 實際操作（IDA 7.0 環境）
- [ ] Maple_Pshark 實際操作（封包 Hook 加密函數）
- [ ] gmh5225/maplestory-v83MaplestoryCPP（C++ 伺服器）編譯與架設

---

**第十七次更新完成（2026-03-24 05:57 UTC）🐱**
