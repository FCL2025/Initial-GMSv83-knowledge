...
> 持續學習中... Last updated: 2026-03-26 (第十七次更新/第三十三次總更新)
...
### 4.3 WZ 修改實戰：移除任務等級限制（2025年最新方法）

**使用場景**：讓低等級角色也能接取/完成某些任務

**所需工具**：
- HaRepacker-resurrected（推薦，GMS old 加密支援）
- Notepad++（文字編輯）
...
3. Tools → Export selection → XML → Classic
   （⚠️ 選 Classic 否則會使用新版加密）
4. 用 Notepad++ 開啟導出的 XML
5. 搜尋所有 lvmax 屬性並移除
   範例：<string lvmax="30"> → <string>
6. 存檔
7. HaRepacker → Tools → Import XML → 選擇修改後的檔案
8. 存檔 Quest.wz
```

**⚠️ 注意事項**：
- 僅修改必要的 lvmax，否則可能破壞任務平衡
- 備份原始 WZ 檔案
- 確認修改後 WZ 版本號正確

**參考來源**：RaGEZONE v83 討論區（2025-07）

### 4.4 WZ 修改實戰：導入新版物品到 v83

**問題**：從新版 MapleStory 導入物品到 v83，spawn 出來無法裝備

**原因**：
- .img 檔案結構不兼容
- 缺少依賴的 WZ 檔案（如 Sound.wz、Quest.wz）
- 物品屬性欄位版本不一致

**解決方案**：
```
1. 確保導入完整的 .img 文件（包括所有依賴）
2. 檢查 Item.wz 的版本號是否匹配
3. 確認 String.wz 中有對應的物品名稱
4. 測試時先在本地環境（非正式伺服器）
```

**參考來源**：RaGEZONE v83 討論區（2025-11）
...
## 四十六、第十七次更新 - MapleSolaxiaV2 完整研究 + Maple_Pshark 抓包工具 + 2026 v83 私人伺服器生態（2026-03-24）

### 46.1 MapleSolaxiaV2 - 經典 v83 伺服器音源（最新研究）

**GitHub:** https://github.com/skhzhang/MapleSolaxiaV2（ronancpl 分支維護版）

**開發者：** Ronan C. P. Lana（獨立自由職开发者）

**專案定位：**
> "The main objective of this project is to try as best as possible to recreate what once was the original MapleStory v83, while adding up some flavors that spices up the gameplay."

**與 Cosmic/HeavenMS 的差異：**
| 特性 | MapleSolaxiaV2 | Cosmic/HeavenMS |
|------|---------------|-----------------|
| 開發工具 | NetBeans 8.0.2（必須）| IntelliJ IDEA |
| JDK 版本 | **JDK/JRE 7（必須）** | JDK 21 |
| 自訂WZ | 大量自訂 WZ（與原版差異大）| 較少自訂（目標是完全移除）|
| WZ 版本 | 基於 Nexon 原版 v83 修改 | 基於 HeavenMS 自訂 |
| 特色 | 遊戲性優化（修復原版問題）| Vanilla gameplay |
| 授權 | 開源（可自由使用/修改/再發布）| 開源 |

**⚠️ 重要限制：必須使用 JDK/JRE 7 和 NetBeans 8.0.2！** 使用更高版本可能導致編譯或執行問題。

### 46.2 MapleSolaxiaV2 完整安裝流程（官方原文）

**第一步：安裝必要工具**
```
1. WampServer2.0i.exe → 伺服器（Web + PHP）
2. hamachi.msi → 虛擬區域網路（多人連線用，可選）
3. mysql-workbench-gpl-5.2.39-win32 → MySQL 資料庫
4. jdk-7u79-windows-x64.exe → JDK 7
5. netbeans-8.0.2-javase-windows.exe → NetBeans IDE
```

**第二步：安裝 JCE（Java 加密擴展）**
```
解壓 jce_policy-7.zip，複製以下檔案到：
...
依序執行 SQL 檔案：
1. db_database.sql → 建立資料庫 schema
2. db_drops.sql → 填充初始資料
可選：db_shopupdate.sql → 更新 NPC 商店（新增原版沒有的商品）
```

**第四步：設定伺服器**
```
1. 將 MapleSolaxia 資料夾放到理想位置（如 C:\Nexon\MapleSolaxia）
2. 編輯 configuration.ini 設定 IP：
   - localhost = 本機使用
   - Hamachi IP = Hamachi 網路
   - 或使用 port-forwarding（本文不介紹）
```

**第五步：在 NetBeans 中開啟專案**
```
1. File > Open Project > 選擇 MapleSolaxia 資料夾
2. 如果有程式碼錯誤：右鍵專案 > Resolve Project Problems
3. 手動指定 "cores" 資料夾位置
...
4. 用客戶端_wz 資料夾中的 WZ 覆蓋原版 WZ
5. 如需修改 IP：用 Neo Hex Editor 開啟 localhost.exe
   Ctrl+F → 搜尋 "127.0.0.1" → 修改為你的 IP
```

**⚠️ 注意：** MapleSolaxiaV2 需要大量自訂 WZ 才能正常運行，必須使用作者提供的 WZ 檔案！

### 46.3 MapleSolaxiaV2 客戶端工具下載（Google Drive）

**連結：** https://drive.google.com/drive/folders/0BzDsHSr-0V4MYVJ0TWIxd05hYUk

**包含內容（推測）：**
- ManagerMsv83.exe → MapleStory 安裝程式
- Localhostv83/ → 本地化客戶端
- client_wz/ → 自訂 WZ 檔案
- free-hex-editor-neo.exe → Hex 編輯器

### 46.4 Maple_Pshark - v83 專用封包攔截工具（最新發現）

**GitHub:** https://github.com/obstriker/Maple_Pshark

**功能說明：**
> "Mapleroyals packet logger. This is a Maplestory v83 packet logger for private servers. This piece of software hooks to the encryption function and let you intercept messages as they go."

**核心功能：**
| 功能 | 說明 |
|------|------|
| **攔截Recv訊息** | Hook 加密函數，即時顯示收到的封包 |
| **修改訊息** | 在傳輸過程中修改封包內容 |
| **注入封包** | 向客戶端或伺服器注入任意封包 |
| **規則過濾** | 基於規則的封包操作和過濾 |

**技術原理：**
- Hook 到 MapleStory 的**加密函數**（不同於 Wireshark 的網路層捕獲）
- 可直接看到解密後的封包內容
- 適用於：分析封包結構、測試伺服器回應、製作作弊工具

**使用場景：**
1. **開發者：** 分析遊戲封包、研究Opcode、建立新功能
2. **伺服器管理員：** 調試伺服器、檢測異常流量
3. **安全研究：** 分析加密機制、發現漏洞

**⚠️ 警告：** 此工具僅用於學習和研究目的。用於盜版或作弊可能違反法律！

### 46.5 MaplePacketPuller - Python 封包結構分析器（最新發現）

**GitHub:** https://github.com/Bratah123/MaplePacketPuller

**功能：** 使用 Python 分析 MapleStory 封包結構
...
### 46.6 Timelapse - MapleStory v83 Trainer（最新發現）

**GitHub:** https://github.com/obstriker/Timelapse-1

**功能：** v83 MapleStory 作弊訓練器

**已知功能：**
- 自動聊天（Auto Chat）
- 自動組隊（Auto Party）
- 自動副本（Auto CC/CS）
- 怪物相關作弊（Miss Godmode、Pet Item Vac）

**⚠️ 警告：** 僅用於學習研究！

### 46.7 2026 年 v83 私人伺服器生態（最新動態）

**新興公開伺服器：**

| 伺服器名稱 | 特色 | URL |
|----------|------|-----|
| **KuroMS** | Pre-BB No P2W, 活躍全球社群 | https://playkuro.com/ |
| TopG v83 列表 | 多個 v83 私人伺服器排名 | topg.org/maplestory-private-servers/version/v83/ |
...
**MapleStory Classic（官方經典版）最新消息：**
- MapleStory Classic World 網站上線（maplestoryclassicworld.com）
- 官方 Classic 仍在 Beta 中
- **2026 年發布可能性極低**（Reddit 消息，2026-02-04）
- 儘管遊戲本身十多年前就已完成

**TopG 2026 年新伺服器列表（2026-03 實測）：**
- V232（新版本）
- v83 Low Rate NoPay2W（懷舊 + 自訂內容）
- v83 Community Balanced NoPay2W（社群平衡）

### 46.8 v83 私人伺服器架設方案總評比（2026 年最新版）

| 方案 | 適合人羣 | 優點 | 缺點 |
|------|---------|------|------|
| **Cosmic v1.1.3** | 進階玩家/開發者 | 最新維護（2026-02）、Liquibase自動遷移、IntelliJ + JDK21 | 需自備客戶端 |
| **HeavenMS** | 進階開發者 | 功能完整、长期维护 | 需要自己整合 |
| **MapleSolaxiaV2** | 懷舊玩家/新手 | 完整客戶端工具包、大量自訂WZ | 需 JDK 7 + NetBeans 8.0.2 |
| **MoopleDev Rev 120** | 經典愛好者 | 純原版 v83 體驗 | 多年無更新 |

**首選推薦（2026 年）：**
1. **Cosmic v1.1.3** - 最佳長期選擇（持續維護到2026年）
2. **MapleSolaxiaV2** - 最完整客戶端方案（一站式安裝）
...
| 工具 | 語言 | 特點 | GitHub |
|------|------|------|--------|
| Maple_Pshark | C# | Hook加密函數，即時解密封包 | obstriker/Maple_Pshark |
| MaplePacketPuller | Python | 自動化封包結構分析 | Bratah123/MaplePacketPuller |
| Timelapse Trainer | C# | v83作弊訓練器 | obstriker/Timelapse-1 |

**WZ 編輯工具：**
| 工具 | 支援版本 | GitHub |
|------|---------|--------|
| HaRepacker-resurrected | 所有版本 | lastbattle/Harepacker-resurrected |
| reWZ | 較新版本 | - |
| Wz2NX | 轉換工具 | - |
...
| 工具 | 用途 | 說明 |
|------|------|------|
| Neo Hex Editor | Hex 編輯 | 替代 HxD 的選擇 |
| STREDIT | 字串編輯 | 脫殼後字串修改 |
| OllyDbg | 調試器 | Themida 脫殼用 |

### 46.10 論壇教程資源（2026-03 最新）

| 資源 | URL | 說明 |
|------|-----|------|
| RaGEZONE v83 完整架設影片 | forum.ragezone.com/threads/video-guide... | 最新 2025 架設教學 |
| MapleSolaxiaV2 原始安裝文 | forum.ragezone.com/f428/maplestory-private-server-v83-741739/ | 經典教程（參考用）|
| MapleStory Client Archive | forum.ragezone.com/threads/maplestory-client-localhost-archive.1101897/ | 所有版本客戶端 |
| localhost Workshop | forum.ragezone.com/threads/localhost-workshop.1202021/ | hendi 客戶端工作坊 |
| Maple Dev Community | discord.gg/DU8j6xrW | 主要 v83 開發者 Discord |
| KuroMS 官方 | https://playkuro.com/ | 2026 年活躍的 v83 伺服器 |

### 46.11 第三次搜尋研究發現

**搜尋引擎限制：**
- RaGEZONE 大部分教程頁面需要登入才能查看
- GitHub 有 rate limiting，連續請求會觸發 429
- Google Drive 連結需要手動下載

**實用建議：**
1. **註冊 RaGEZONE 帳號** - 可訪問所有教程和資源
2. **申請 Brave Search API Key** - 提高搜尋精確度
3. **使用 Git LFS** - Clone 大型 repo 時更穩定

---

## 🔄 更新記錄（2026-03-24 第十七次更新）

### 第十七次更新 - MapleSolaxiaV2 + Maple_Pshark + 2026生態
- [x] 研究 MapleSolaxiaV2 完整內容（ronancpl 分支維護版）
- [x] 補充 MapleSolaxiaV2 完整安裝流程（JDK 7 + NetBeans 8.0.2）
- [x] 補充 MapleSolaxiaV2 自訂 WZ 差異說明
- [x] 新增 Maple_Pshark 封包攔截工具（Hook 加密函數，v83專用）
- [x] 新增 MaplePacketPuller Python 封包分析器
- [x] 新增 Timelapse Trainer v83 作弊訓練器
- [x] 新增 KuroMS 2026 年活躍 v83 公開伺服器
- [x] 新增 MapleStory Classic 官方動態（2026 年發布可能性極低）
- [x] 更新 v83 私人伺服器架設方案總評比
- [x] 新增 2026 年最新工具發現（Maple_Pshark、MaplePacketPuller、Timelapse）
- [x] 更新論壇教程資源列表

### 第十七次更新完成（2026-03-24 07:57 UTC）🐱

---

**第十七次更新完成 ✅**
- 學習時間：2026-03-24 07:57 UTC
- 新增章節：四十六
- 重點更新：MapleSolaxiaV2、Maple_Pshark、2026 v83 生態
